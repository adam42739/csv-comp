...
static void u64_string_trim_lz(char *string)
{
    int len = strlen(string);
    for (int i = 0; i < len; ++i)
    {
...
static int load_u8diff(void *bytes, int *index, int size, char **col, int *row)
{
    int success = TRUE;
    u16 num = 0;
    success = bytes_load(bytes, index, size, (void *)&num, U16);
    if (!success)
    {
        return FALSE;
    }
...
static int load_u16diff(void *bytes, int *index, int size, char **col, int *row)
{
    int success = TRUE;
    u16 num = 0;
    success = bytes_load(bytes, index, size, (void *)&num, U16);
...
static int load_u32diff(void *bytes, int *index, int size, char **col, int *row)
{
    int success = TRUE;
    u16 num = 0;
    success = bytes_load(bytes, index, size, (void *)&num, U16);
...
static int load_u64diff(void *bytes, int *index, int size, char **col, int *row)
{
    int success = TRUE;
    u16 num = 0;
    success = bytes_load(bytes, index, size, (void *)&num, U16);
...
static int u8_possible(struct frame *df, int row, int col, u8 *factor, u8 *max_factor)
{
    if (uint_possible(df, row, col))
    {
        ...
        else
        {
            if (value <= UINT8_MAX)
            {
                if (value == 0)
                {
                    *max_factor = UINT8_MAX;
                }
                else
                {
                    *max_factor = *factor;
                    while (value * 10 < UINT8_MAX)
                    {
                        value *= 10;
                        ++*max_factor;
                    }
...
static int u16_possible(struct frame *df, int row, int col, u8 *factor, u8 *max_factor)
{
    if (uint_possible(df, row, col))
    {
        u64 value;
...
static int u32_possible(struct frame *df, int row, int col, u8 *factor, u8 *max_factor)
{
    if (uint_possible(df, row, col))
    {
        u64 value;
...
static int u64_possible(struct frame *df, int row, int col, u8 *factor, u8 *max_factor)
{
    if (uint_possible(df, row, col))
    {
        u64 value;
...
static float comp_u8same(struct frame *df, int row, int col)
{
    u8 factor;
    u8 max_factor;
    if (u8_possible(df, row, col, &factor, &max_factor))
    {
        int num_same = num_strsame(df, row, col);
        if (num_same > UINT16_MAX)
        {
...
static int get_u8_poss_num(struct frame *df, int row, int col, u8 *max_factor, u8 *factor)
{
    if (u8_possible(df, row, col, factor, max_factor))
    {
        int num_poss = 0;
        while (row + num_poss < df->num_rows)
        {
            u8 new_factor;
            u8 new_max_factor;
            if (u8_possible(df, row + num_poss, col, &new_factor, &new_max_factor))
            {
...
static float comp_u8diff(struct frame *df, int row, int col)
{
    u8 factor;
    u8 max_factor;
    int num_poss = get_u8_poss_num(df, row, col, &max_factor, &factor);
    if (num_poss > 0)
    {
        if (num_poss > UINT16_MAX)
        {
            num_poss = UINT16_MAX;
        }
        int high_size = 0;
        ...
        {
            high_size += strlen(df->cols[col][row + i]);
        }
        int low_size = sizeof(u16) + sizeof(u8) + sizeof(u8) * num_poss;
        return (((float)low_size) / ((float)high_size));
    }
    else
    {
...
static float comp_u16same(struct frame *df, int row, int col)
{
    u8 factor;
    u8 max_factor;
    if (u16_possible(df, row, col, &factor, &max_factor))
    {
        int num_same = num_strsame(df, row, col);
        if (num_same > UINT16_MAX)
        {
...
static int get_u16_poss_num(struct frame *df, int row, int col, u8 *max_factor, u8 *factor)
{
    if (u16_possible(df, row, col, factor, max_factor))
    {
        int num_poss = 0;
        while (row + num_poss < df->num_rows)
        {
            u8 new_factor;
            u8 new_max_factor;
            if (u16_possible(df, row + num_poss, col, &new_factor, &new_max_factor))
            {
                if (new_factor <= *max_factor)
                {
                    if (new_factor > *factor)
...
static float comp_u16diff(struct frame *df, int row, int col)
{
    u8 factor;
    u8 max_factor;
    int num_poss = get_u16_poss_num(df, row, col, &max_factor, &factor);
    if (num_poss > 0)
    {
        if (num_poss > UINT16_MAX)
        {
        ...
        int low_size = sizeof(u16) + sizeof(u8) + sizeof(u16) * num_poss;
        return (((float)low_size) / ((float)high_size));
    }
    else
    {
...
static float comp_u32same(struct frame *df, int row, int col)
{
    u8 factor;
    u8 max_factor;
    if (u32_possible(df, row, col, &factor, &max_factor))
    {
        int num_same = num_strsame(df, row, col);
        if (num_same > UINT16_MAX)
        {
...
static int get_u32_poss_num(struct frame *df, int row, int col, u8 *max_factor, u8 *factor)
{
    if (u32_possible(df, row, col, factor, max_factor))
    {
        int num_poss = 0;
        while (row + num_poss < df->num_rows)
        {
            u8 new_factor;
            u8 new_max_factor;
            if (u32_possible(df, row + num_poss, col, &new_factor, &new_max_factor))
            {
                if (new_factor <= *max_factor)
                {
                    if (new_factor > *factor)
...
static float comp_u32diff(struct frame *df, int row, int col)
{
    u8 factor;
    u8 max_factor;
    int num_poss = get_u32_poss_num(df, row, col, &max_factor, &factor);
    if (num_poss > 0)
    {
        if (num_poss > UINT16_MAX)
        {
        ...
        int low_size = sizeof(u16) + sizeof(u8) + sizeof(u32) * num_poss;
        return (((float)low_size) / ((float)high_size));
    }
    else
    {
...
static float comp_u64same(struct frame *df, int row, int col)
{
    u8 factor;
    u8 max_factor;
    if (u64_possible(df, row, col, &factor, &max_factor))
    {
        int num_same = num_strsame(df, row, col);
        if (num_same > UINT16_MAX)
        {
...
static int get_u64_poss_num(struct frame *df, int row, int col, u8 *max_factor, u8 *factor)
{
    if (u64_possible(df, row, col, factor, max_factor))
    {
        int num_poss = 0;
        while (row + num_poss < df->num_rows)
        {
            u8 new_factor;
            u8 new_max_factor;
            if (u64_possible(df, row + num_poss, col, &new_factor, &new_max_factor))
            {
                if (new_factor <= *max_factor)
                {
                    if (new_factor > *factor)
...
static float comp_u64diff(struct frame *df, int row, int col)
{
    u8 factor;
    u8 max_factor;
    int num_poss = get_u64_poss_num(df, row, col, &max_factor, &factor);
    if (num_poss > 0)
    {
        if (num_poss > UINT16_MAX)
        {
        ...
        int low_size = sizeof(u16) + sizeof(u8) + sizeof(u64) * num_poss;
        return (((float)low_size) / ((float)high_size));
    }
    else
    {
...
static void dump_u8diff(struct frame *df, void **bytes, int *index, int *size, int *row, int col)
{
    ...
    u8 indchr = (u8)U8_DIFFS;
    bytes_dump(bytes, index, size, (void *)&indchr, U8);
    u16 num = (u16)num_poss;
    bytes_dump(bytes, index, size, (void *)&num, U16);
    bytes_dump(bytes, index, size, (void *)&factor, U8);
    for (int i = 0; i < num_poss; ++i)
    {
        u8 new_factor;
        u64 u_int;
        (void)string_to_u64(df->cols[col][*row], &u_int, &new_factor);
